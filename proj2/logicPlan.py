...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]] = None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # Current <==> (previous position at time t-1) & (took action to move to x, y)
    move_causes_sent = disjoin(possible_causes)
    return conjoin([PropSymbolExpr(pacman_str, x, y, time=now) % move_causes_sent])
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None,
    ...
    "*** BEGIN YOUR CODE HERE ***"
    impossible_pacman_pos = []
    for pac_pos in all_coords:
        x, y = pac_pos
        impossible_pacman_pos.append(PropSymbolExpr(wall_str, x, y) >> ~PropSymbolExpr(pacman_str, x, y, time=t))

    # possible pacman position: [A and ~B and ~C and ..., ~A and B and ~C and ..., ...]
    possible_pacman_pos = [PropSymbolExpr(pacman_str, pos[0], pos[1], time=t) for pos in non_outer_wall_coords]
    pacman_pos_sentences = conjoin([exactlyOne(possible_pacman_pos)] + impossible_pacman_pos)

    # possible pacman direction: [North and ~South and ~East and ~ West, ~North and ..., ..]
    possible_pacman_dir = [PropSymbolExpr(pac_dir, time=t) for pac_dir in DIRECTIONS]
    pacman_dir_sentences = exactlyOne(possible_pacman_dir)

    pacphysics_sentences.append(pacman_pos_sentences)
    pacphysics_sentences.append(pacman_dir_sentences)
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    KB.append(conjoin(list(map(
        lambda pac_pos: PropSymbolExpr(pacman_str, pac_pos[0], pac_pos[1], time=0)
        if pac_pos == (x0, y0)
        else ~PropSymbolExpr(pacman_str, pac_pos[0], pac_pos[1], time=0),
        all_coords))))

    # action0
    KB.append(conjoin(list(map(
        lambda dir: PropSymbolExpr(dir, time=0)
        if dir == action0
        else ~PropSymbolExpr(dir, time=0),
        DIRECTIONS))))

    # action1
    KB.append(conjoin(list(map(
        lambda dir: PropSymbolExpr(dir, time=1)
        if dir == action1
...
def mapping(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # add initial location to KB, also wall information
    KB.append(conjoin([PropSymbolExpr(pacman_str, x, y, time=0) if (x, y) == (pac_x_0, pac_y_0)
                       else ~PropSymbolExpr(pacman_str, x, y, time=0) for x, y in all_coords]))
    ...
    for t in range(agent.num_timesteps):
        ...
        # add pacman physic, action and percept to KB
...
def slam(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # add initial location to KB, and whether there is a wall at that location
    KB.append(conjoin([PropSymbolExpr(pacman_str, x, y, time=0) if (x, y) == (pac_x_0, pac_y_0)
                       else ~PropSymbolExpr(pacman_str, x, y, time=0) for x, y in all_coords]))
    KB.append(~PropSymbolExpr(wall_str, pac_x_0, pac_y_0))
    known_map[pac_x_0][pac_y_0] = 0
